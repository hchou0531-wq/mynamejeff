...
export async function sendEmail({ to, subject, html, text, attachments }) {
  ...
  try {
    const res = await fetch(RESEND_API, {
      ...
      body: JSON.stringify({ from: process.env.EMAIL_FROM, to: [to], subject, html, text, ...(attachments?.length ? { attachments } : {}) }),
    })
    if (!res.ok) {
...
// Read once per cold start rather than per send — the file never changes at runtime, and
// a serverless instance sends many verification emails over its lifetime.
let wordmarkAttachmentPromise = null
async function wordmarkAttachment() {
  if (!wordmarkAttachmentPromise) {
    wordmarkAttachmentPromise = readFile(path.join(process.cwd(), 'public', 'wordmark.png'))
      .then(buf => ({ filename: 'wordmark.png', content: buf.toString('base64'), content_id: 'wordmark' }))
      .catch(e => {
        console.error('[email] Could not read public/wordmark.png for inline embedding:', e.message)
        return null
      })
  }
  return wordmarkAttachmentPromise
}
...
export async function verificationEmailTemplate(code, siteUrl) {
  ...
  const url = siteUrl || ''
  // Embedded as a cid: attachment rather than an <img src="{url}/wordmark.png">: the old
  // approach depended on the site being publicly reachable at send time (broken by e.g. a
  // local dev tunnel that's down, mid-restart, or pointed at a stale URL) purely to show a
  // logo. Inline images travel with the email itself, so they render with zero dependency
  // on any server being up.
  const wordmark = await wordmarkAttachment()
  const text = [
...
<div style="background:#0a0912;padding:40px 16px;font-family:'VT323',ui-monospace,'Courier New',monospace;">
  <div style="max-width:420px;margin:0 auto;background:#140a24;border:3px solid #6b21a8;padding:0;">
    <div style="padding:28px 28px 24px;">
      <div style="text-align:center;margin:0 0 20px;">${
        wordmark
          ? `<img src="cid:${wordmark.content_id}" alt="Ethereal" width="220" style="display:inline-block;width:220px;height:auto;">`
          // public/wordmark.png couldn't be read (logged above) — fall back to plain text
  ...
  return { subject: 'Verify your Ethereal account', text, html, attachments: wordmark ? [wordmark] : [] }
}
